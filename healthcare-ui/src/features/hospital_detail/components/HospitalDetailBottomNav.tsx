...
const navItems = [
  ...
  { icon: 'health_and_safety', label: 'HealthCare+', href: '/' },
];

export const HospitalDetailBottomNav: React.FC = () => {
  return (
      ...
      <div className="max-w-lg mx-auto flex items-center justify-around py-2">
        {navItems.map((item) => (
          <Link
            key={item.label}
            href={item.href}
            className="flex flex-col items-center gap-0.5 px-3 py-1 text-on-surface-variant hover:text-primary transition-colors"
          >
